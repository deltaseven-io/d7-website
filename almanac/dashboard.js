...
function muCell(p) {
  ...
  if (p.xslg != null)
    parts.push(
      `xSLG ${p.xslg < 1 ? p.xslg.toFixed(3) : (p.xslg / 1000).toFixed(3)}`,
    );
  if (p.xhr_gap != null)
    parts.push(`xHR gap: ${p.xhr_gap > 0 ? "+" : ""}${p.xhr_gap}`);
  if (p.is_danger) parts.push("☄️ HR BLAST");
  if (parts.length) tip += `\n${parts.join(" · ")}`;
  if (p.p_vuln_score != null)
...
function tierIcon(p) {
  ...
  const vulnBadge =
    vuln != null
      ? `<span style="font-size:8px;color:${vuln >= 0.55 ? "#f85149" : vuln <= 0.35 ? "#58a6ff" : "#555"};display:block" title="Pitcher vulnerability ${(vuln * 100).toFixed(0)}%">${(vuln * 100).toFixed(0)}%</span>`
      : "";
  if (bt === "gascan")
  ...
  const t = p.sp_tier || "";
  if (t === "ELITE")
    return `<span title="Elite${vulnTip}">⭐⭐</span>${vulnBadge}`;
  if (t === "ACE") return `<span title="Ace${vulnTip}">⭐</span>${vulnBadge}`;
  if (t === "VULNERABLE")
    return `<span title="Weak SP${vulnTip}">🩸</span>${vulnBadge}`;
  if (t === "BLEEDING")
    return `<span title="V.Weak SP${vulnTip}">🩸🩸</span>${vulnBadge}`;
  return vulnBadge;
}
function logoImg(p) {
...
function rebuildAccPanel() {
  ...
  if (oversEl && undersEl) {
    const ouGames = (gp.recent || []).filter(
      (g) => g.est_runs && g.actual_runs != null,
    );
    if (ouGames.length) {
      ...
      const overColor =
        overPct >= 55 ? "#00c853" : overPct >= 45 ? "#c9a800" : "#f85149";
      const underColor =
        underPct >= 55 ? "#00c853" : underPct >= 45 ? "#c9a800" : "#f85149";
...
function buildStrip() {
  ...
  GAMES.forEach((g, i) => {
    ...
    const tagColor =
      g.game_tag === "Favorable"
        ? "#00c853"
        : g.game_tag === "Unfavorable"
          ? "#f85149"
          : "#555";
    const boostStr = g.park_boost ? ` · ${g.park_boost.toFixed(2)}x boost` : "";
    const envTag = g.game_tag
      ? `<span style="display:block;font-size:11px;color:${tagColor};margin-top:2px">${g.game_tag === "Favorable" ? "🟢" : g.game_tag === "Unfavorable" ? "🔴" : "⚪"} ${g.game_tag}${boostStr}</span>`
      : "";
