...
function pcell(v) {
  const bg =
    v >= 50 ? "#00c853" : v >= 35 ? "#c9a800" : v >= 20 ? "#cb4335" : "#a93226";
  const fg = v >= 50 ? "#fff" : v >= 35 ? "#000" : v >= 20 ? "#000" : "#000";
  return `<span class="hcell" style="background:${bg};color:${fg}">${(+v).toFixed(1)}%</span>`;
}
function predCell(v, dec = 2) {
  if (v == null || v === undefined) return '<span class="na-cell">—</span>';
  const n = +v;
  ...
  return `<span style="display:block;width:100%;height:38px;line-height:36px;text-align:center;font-weight:700;font-size:13px;border:1px solid ${color};color:${textColor};background:${color}${bgOpacity};box-sizing:border-box">${n.toFixed(dec)}</span>`;
}
function tc(v) {
...
function bloodTier(p) {
  const sp = p.sp_tier || "",
    bp = p.bp_tier || "",
    wm = p.wind_mult || 1.0;
  if (sp === "BLEEDING" && bp === "BLEEDING" && wm >= 1.1) return "gascan";
...
function tierIcon(p) {
  ...
  if (bt === "gascan")
    return '<span title="SP🩸⛽: both BLEEDING + wind out">🩸⛽</span>';
  if (bt === "dbl")
    return '<span title="SP🩸🩸+BP🩸🩸: both BLEEDING">🩸🩸</span>';
  if (bt === "both_vuln")
    return '<span title="SP🩸+BP🩸: both VULNERABLE">🩸</span>';
  const t = p.sp_tier || "";
  if (t === "ELITE") return '<span title="Elite">⭐⭐</span>';
  if (t === "ACE") return '<span title="Ace">⭐</span>';
  if (t === "VULNERABLE") return '<span title="Weak SP">🩸</span>';
  if (t === "BLEEDING") return '<span title="V.Weak SP">🩸🩸</span>';
  return "";
}
function logoImg(p) {
...
function buildBloodCol(elId, picks) {
  ...
  if (!picks || picks.length === 0) {
    el.innerHTML = '<div style="color:#333;font-size:9px">no data yet</div>';
    return;
  }
...
function rebuildAccPanel() {
  ...
  function buildMUCard(elId, label, graded, lo, hi) {
    ...
    if (!el) return;
    el.style.cssText =
      "background:#0d1117;border:1px solid #1e1e1e;border-radius:8px;padding:12px 14px;text-align:center";
    function _w(cutoff) {
      const rows = graded.filter((p) => {
        const mu = approxMU(p);
        return (
    ...
    const last5 = _w(d(5));
    const pctColor = (p) =>
      p >= 65 ? "#00c853" : p >= 50 ? "#c9a800" : p >= 1 ? "#f85149" : "#555";
  ...
  const allTopHrr = (A.daily_picks || []).flatMap((d) =>
    (d.top_hrr || []).filter((p) => p.graded && p.hit_hrr !== null),
  );
  const muLabels = [
    { l: "80+", lo: 80, hi: 999 },
    { l: "70-79", lo: 70, hi: 80 },
  ...
  if (ehrEl) {
    const pctColor2 = (p) =>
      p >= 65 ? "#00c853" : p >= 50 ? "#c9a800" : "#f85149";
    ehrEl.innerHTML = `<div style="display:flex;gap:8px;flex-wrap:wrap">${muLabels
      .map((b) => {
        const rows = allTopHrr.filter((p) => {
          const mu = approxMU(p);
          return mu >= b.lo && mu < b.hi;
        });
        if (!rows.length)
  ...
  function teamName(g, side) {
    return g[side] && isNaN(g[side])
      ? g[side]
      : TEAM_ABBR[g[side + "_id"]] || g[side + "_id"] || "?";
  }
  ...
  [
    ["vstrong", "acc-games-vstrong"],
    ["strong", "acc-games-strong"],
    ["moderate", "acc-games-moderate"],
    ["tossup", "acc-games-tossup"],
  ].forEach(([tier, elId]) => {
    const el = document.getElementById(elId);
    if (!el) return;
  ...
  if (rdEl)
    rdEl.textContent = gp.runs_avg_diff != null ? `±${gp.runs_avg_diff}` : "—";
  // O/U prediction accuracy — did our est_runs correctly predict over/under the line?
  const ouEl = document.getElementById("acc-games-ou");
  if (ouEl) {
    const ouGames = (gp.recent || []).filter(
      (g) => g.ou_line && g.est_runs && g.actual_runs != null,
    );
    let ouHits = 0,
      ouTotal = 0;
    ouGames.forEach((g) => {
      const estSide =
        g.est_runs > g.ou_line ? "O" : g.est_runs < g.ou_line ? "U" : "P";
      const actSide =
        g.actual_runs > g.ou_line ? "O" : g.actual_runs < g.ou_line ? "U" : "P";
      if (estSide === "P" || actSide === "P") return;
      ouTotal++;
      if (estSide === actSide) ouHits++;
    });
    if (ouTotal) {
      const ouPct = Math.round((100 * ouHits) / ouTotal);
      const ouColor =
        ouPct >= 55 ? "#00c853" : ouPct >= 45 ? "#c9a800" : "#f85149";
      ouEl.innerHTML = `<div style="font-size:22px;font-weight:700;font-family:'Barlow Condensed',sans-serif;color:${ouColor}">${ouHits}/${ouTotal}</div><div style="font-size:10px;color:#555">${ouPct}%</div>`;
    } else {
      ouEl.innerHTML =
        '<div style="color:#555;font-size:10px">starts today</div>';
    }
  }
  ...
  if (recentEl) {
    ...
    if (dates.length) {
      recentEl.innerHTML =
        `<div style="display:grid;grid-template-columns:80px 1fr 1fr 1fr 1fr 60px;gap:4px;font-size:10px;margin-bottom:4px">
      ...
      recentEl.innerHTML =
        '<div style="color:#555;font-size:10px">no graded games yet</div>';
    }
  }
  ...
  if (carnageEl) {
    if (carnageData.length) {
      carnageEl.innerHTML = carnageData
        .slice(0, 30)
        .map((c) => {
      ...
      carnageEl.innerHTML =
        '<div style="color:#555;font-size:10px">no carnage data yet</div>';
    }
  }
...
function setSort(c) {
  ...
  document.querySelectorAll('thead th[id^="h-"]').forEach((th) => {
    ...
    if (k === col) {
      ...
      th.textContent =
        th.textContent.replace(/[ ▾▴]/g, "").trim() + (dir === 1 ? " ▾" : " ▴");
    } else {
...
function streakBadge(p) {
  ...
  if (s <= 0)
    return '<span style="color:rgba(255,255,255,0.1);font-size:10px">—</span>';
  const color =
    s >= 7 ? "#ffd700" : s >= 5 ? "#00c853" : s >= 3 ? "#58a6ff" : "#7d8590";
...
function compositeScore(p) {
  ...
  const trendAdj = trend > 0.5 ? 2 : trend > 0 ? 1 : trend < -0.5 ? -2 : -1;
...
function p1Score(p) {
  ...
  const trend = p.trend || 0;
  const trendAdj =
    trend > 0.5 ? +4 : trend > 0 ? +2 : trend < -0.5 ? -4 : trend < 0 ? -2 : 0;
  ...
  const wm = p.wind_mult || 1.0;
  const weatherAdj =
    wm >= 1.2 ? +2 : wm >= 1.1 ? +1 : wm <= 0.9 ? -2 : wm <= 0.96 ? -1 : 0;
...
function adjStreak(type, delta) {
  ...
  if (type === "h") streakMin = Math.max(1, Math.min(8, streakMin + delta));
...
function render() {
  ...
  let rows = Object.values(src).filter((p) => p.games >= minG);
  if (q)
    rows = rows.filter((p) => p.name.toLowerCase().includes(q.toLowerCase()));
  // Exclude bench players when lineup is confirmed
  rows = rows.filter((p) => !(p.lineup_confirmed && p.lineup_order === null));
  if (streakFilter)
    rows = rows.filter((p) => getStreak(p.log || [], streakMin) >= streakGames);
  if (streakHide)
    rows = rows.map((p) => ({
      ...p,
  ...
  const sv = (r) => {
    if (col === "best") return bestStreak(r.log_full || r.log || [], streakMin);
    if (col === "avgstreak")
  ...
  tb.innerHTML = rows
    ...
    <td class="name-td"><div class="player-cell">${logoImg(p)}<span class="pname">${p.name}</span>${windBadge(p)}${accBadge(p)}</div></td>
  ...
  document.getElementById("taccn").textContent =
    acc.total > 0 ? acc.hits + "/" + acc.total + " correct" : "no data yet";
  // Top-10 W/L tiles
...
function buildStrip() {
  ...
  GAMES.forEach((g, i) => {
    ...
    pill.innerHTML = `${wxBadge(g)}<strong>${aL}${g.away} @ ${hL}${g.home}</strong><span>${g.away_sp} vs ${g.home_sp} · ${time} CT</span>${edgeStr}`;
    pill.onclick = () => {
...
function wxBadge(g) {
  ...
  if (mph > 0)
    lines.push(
      "<span" + (cls ? ' class="' + cls + '"' : "") + ">" + mph + " mph</span>",
    );
  if (g.wind_label) {
...
document
  .querySelectorAll(".btn[data-s]")
  .forEach((b) => b.addEventListener("click", () => setSort(b.dataset.s)));
